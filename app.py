...
# 📌 App 內常駐廣告條的 AdSense 廣告單元 slot ID。
# AdSense 帳號審核通過前無法建立廣告單元、拿不到真正的 slot ID，
# 所以先留空（''），index.html 會依此值自動不渲染 <ins> 標籤，避免用假值送出無效廣告請求。
# 審核通過後，去 AdSense 後台「廣告 > 依廣告單元」建立橫幅廣告單元，
# 把拿到的 slot ID 填進 .env 的 BANNER_AD_SLOT_ID 即可自動生效，不用再改程式碼。
BANNER_AD_SLOT_ID = os.getenv('BANNER_AD_SLOT_ID', '')
...
def index():
    ...
    if os.getenv("SINGLE_USER_MODE", "false").lower() == "true":
        ...
        return render_template(
            ...
            banner_ad_slot_id=BANNER_AD_SLOT_ID,
            **urls
        )
    ...
    # 🌟 未登入訪客：顯示公開行銷首頁（供 Google 爬蟲審核與新使用者了解產品），不再直接曝光登入框
    if not logged_in:
...
@app.route('/login')
def login_page():
    # 已登入使用者造訪 /login，直接導回 App 主介面，不重複顯示登入卡
    if 'credentials' in session:
        return redirect('/')
